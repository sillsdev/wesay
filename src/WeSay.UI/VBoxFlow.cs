...
namespace WeSay.UI
{
	public class FlexibleHeightPanel : Panel
	{
		...
		{
			this.Size = new Size(initialWidth, controllingChild.Height + _heightDifferenceFromControllingChild);
...
//            this.BackColor = Color.Wheat;
		...
		void OnChildResize(object sender, EventArgs e)
		{
			...
			{
				this.Height = c.Height + _heightDifferenceFromControllingChild;
			}
		}
	}

	public partial class VBoxFlow : TableLayoutPanel//, IContainerControl
	{
		/// <summary>
		...
		public VBoxFlow()
		{
			...
			InitializeComponent();
			this.ColumnCount = 1;// = FlowDirection.TopDown;
...
//        }
		...
		void OnChildControlResized(object sender, EventArgs e)
		{
			...
			this.Height = h + this.Margin.Top + this.Margin.Bottom+ (5 * (ActualControls.Count - 1));
		}
