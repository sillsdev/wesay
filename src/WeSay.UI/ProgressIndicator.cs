  ...
  public partial class ProgressIndicator : UserControl
  {
	...
	public int Maximum
	{
	  ...
	  set
	  {
		if(value < _minimum)
		{
		  throw new ArgumentOutOfRangeException("Maximum must not be less than Minimum");
		}
		_maximum = value;
	...
	public int Minimum
	{
	  ...
	  set
	  {
		if (value < 1)
		{
		  throw new ArgumentOutOfRangeException("Minumum must not be less than 1");
		}
		if (value > _maximum)
		{
	...
	public int Rows
	{
	  ...
	  set
	  {
		if(value < 1)
		{
		  throw new ArgumentOutOfRangeException("Value must not be less than 1");
		}
		_rows = value;
		if (AutoSize)
		{
	...
	public Size CellSize
	{
	  get
	  {
		return Size.Add(_bulletSize, _bulletPadding.Size);
	  }
	}
	...
	public Color BulletColorEnd
	{
	  get
	  {
		return this._bulletColorEnd;
	  }
	  set
	  {
		this._bulletColorEnd = value;
		Invalidate();
	  }
	}
	...
	{
	  get { return this._highlightCurrentOnly; }
	  set { this._highlightCurrentOnly = value; Invalidate();}
	}
	...
	protected override void OnPaint(PaintEventArgs e)
	{
	  ...
	  for (int column = 0; column < columnCount; column++)
	  {
		for (int row = 0; row < Rows; row++)
		{
		  ...
		  Rectangle rect = new Rectangle(location.X, location.Y, BulletSize.Width, BulletSize.Height);
	...
	{
	  int remainder;
	  int columnCount = Math.DivRem(this._maximum, this.Rows, out remainder);
	  if(remainder != 0)
	  {
		++columnCount;
	  }
