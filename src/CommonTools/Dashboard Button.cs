...
namespace WeSay.CommonTools
{
	public partial class DashboardButton: UserControl
	{
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			//draw shadow
			Rectangle rectangle =
					new Rectangle(ClientRectangle.Left + shadowWidth,
			...
			{
				rectangle =
						new Rectangle(ClientRectangle.Left + 2,
		...
		protected virtual void PaintContents(PaintEventArgs e)
		{
			...
			if (task != null && task.GetReferenceCount() > 0 && task.GetRemainingCount() >= 0)
			{
				...
				float rightEdgeOfDonePart = (float) (percentDone / 100.0) * (rightEdge - left) +
											left;
			...
			e.Graphics.DrawString(Text, Font, Brushes.Black, left + nudge, 10 + nudge);
		}
