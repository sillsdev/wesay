...
namespace WeSay.LexicalTools.AddMissingInfo
{
	public class MissingInfoViewMaker
	{
		...
		{
			var template = CreateViewTemplateFromListOfFields(baseTemplate, config.FieldsToShow, config.FieldsToShowReadOnly);
			...
			var isBaseFormFillingTask = config.FieldsToShow.Contains(LexEntry.WellKnownProperties.BaseForm);
