		...
		public static Font CreateFont(WritingSystemDefinition writingSystem)
		{
			...
			catch (Exception e) //I could never get the above to fail, but a user did (WS-34091), so now we catch it here.
			{
				ErrorReport.NotifyUserOfProblem(new ShowOncePerSessionBasedOnExactMessagePolicy(),
					e,
					"There is something wrong with the font {0} on this computer. Try re-installing it. Meanwhile, the font {1} will be used instead.", writingSystem.DefaultFont.Name, SystemFonts.DefaultFont.Name);
