...
namespace WeSay.Project.Tests
{
	...
	public class CacheBuilderTests
	{
		...
		private readonly string _simpleGoodLiftContents =
				string.Format(
		...
		{
			get { return WeSayWordsProject.Project.PathToLiftBackupDir; }
		}
		...
		public void WorksWithTempDirectoryOnADifferentVolumne()
		{
			...
			else
			{
				...
				{
					if (drive.IsReady && drive.DriveType != DriveType.CDRom && drive.Name != "C:\\")
					{
		...
		{
			File.WriteAllText(_cacheBuilder.SourceLIFTPath,
							  _simpleGoodLiftContents.Replace("</lift>", "<x/></lift>"));
			Assert.AreEqual(ProgressState.StateValue.NotStarted, _progress.State);
			_cacheBuilder.DoWork(_progress);
			// WaitForFinish();
		}

		private void OnLog(object sender, ProgressState.LogEvent e)
		{
			_log += e.message;
		}
		...
		[Test]
		[Ignore("Do we really need a backup of the cache?")]
